...
int main()
{
    ...
    fclose(fp) ;
    fclose(yarooha) ;
    struct afrad x1,x2,x3;
    struct afrad *px1,*px2 , *px3 ;
    px1=&x1;
    px2=&x2;
    px3=&x3;
    if(chekonim==4)   /*show the ranking*/
    {
        ...
        for(j=0;j<3;j++)
        {
            fread(px1,sizeof(struct afrad),1,yarooha);
            while(!feof(yarooha))
            {

                fread(px2,sizeof(struct afrad),1,yarooha);
                if(px2->emtiaz>px1->emtiaz)
                {
                    fseek(yarooha,-sizeof(struct afrad),SEEK_CUR);
                    fwrite(px1,sizeof(struct afrad),1,yarooha);
                    fseek(yarooha,j*sizeof(struct afrad),SEEK_SET);
                    fwrite(px2,sizeof(struct afrad),1,yarooha);
            ...
            fseek(yarooha,(j+1)*sizeof(struct afrad),SEEK_SET);
        }
        fseek(yarooha,0,SEEK_SET) ;
        for(j=0;j<3;j++)
        {
            fread(px1,sizeof(struct afrad),1,yarooha);
            printf("%d . %s ba emtiaze %lf to %s\n",j+1,px1->nam,px1->emtiaz,px1->topic);
        }
